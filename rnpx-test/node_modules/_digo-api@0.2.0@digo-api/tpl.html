...
<body>
	<aside id="api_sidebar">
		<input type="search" id="api_filter" placeholder="搜索..." oninput="setFilter(this.value)">
		<ul>
			$side
		</ul>
	</aside>
	...
	<script>
		...
		function setFilter(value) {
			var items = document.querySelectorAll('#api_sidebar li')
			for (var i = 0; i < items.length; i++) {
				...
				item.style.display = ''
				if (item.firstChild.title.indexOf(value) < 0) {
			...
			for (var i = 0; i < items.length; i++) {
				...
				item.style.display = ''
				if (!item.firstChild.nextSibling.offsetHeight) {
					item.style.display = 'none'
				}
			}
		}
		...
		function ajax(options) {
			...
			if (options.type !== "GET" && options.contentType) {
