...
(function (ConsoleColor) {
     ...
     * 亮色。
     */
    ConsoleColor[ConsoleColor["bright"] = 1] = "bright";
    /**
     * 黑色。
     */
    ConsoleColor[ConsoleColor["brightBlack"] = 90] = "brightBlack";
    /**
     * 亮红色。
     */
    ConsoleColor[ConsoleColor["brightRed"] = 91] = "brightRed";
    /**
     * 亮绿色。
     */
    ConsoleColor[ConsoleColor["brightGreen"] = 92] = "brightGreen";
    /**
     * 亮黄色。
     */
    ConsoleColor[ConsoleColor["brightYellow"] = 93] = "brightYellow";
    /**
     * 亮蓝色。
     */
    ConsoleColor[ConsoleColor["brightBlue"] = 94] = "brightBlue";
    /**
     * 亮紫色。
     */
    ConsoleColor[ConsoleColor["brightMagenta"] = 95] = "brightMagenta";
    /**
     * 亮靛色。
     */
    ConsoleColor[ConsoleColor["brightCyan"] = 96] = "brightCyan";
    /**
     * 亮白色。
     */
    ConsoleColor[ConsoleColor["brightWhite"] = 97] = "brightWhite";
 ...
 * @return 返回已处理的字符串。
 */
function addLogColor(value, color) {
    return `\u001b[${color}m${value}\u001b[${color === 1 /* bright */ ? 0 : 39}m`;
}
exports.addLogColor = addLogColor;
/**
 * 删除日志字符串中的所有控制字符。
 * @param value 要处理的字符串。
 * @return 返回已处理的字符串。
 */
function removeLogColor(value) {
    return value.replace(controlRegex, "");
}
exports.removeLogColor = removeLogColor;
/**
 * 格式化指定的日志字符串。
 ...
 */
function formatLog(value, args) {
    return value.replace(/\{(?:([a-zA-Z]+):)?([^}]+)\}/g, (all, color, key) => {
 ...
 * @return 返回已处理的字符串。
 */
function ellipsisLog(value, width = 0) {
    ...
    const controls = []; // [开始位置1, 结束位置1, 开始位置2, ...]
    value.replace(controlRegex, (all, index) => {
...
exports.ellipsisLog = ellipsisLog;
/**
 * 将指定的日志字符串按最大宽度拆成多行。
 * @param value 要处理的字符串。
 * @param width 允许的最大宽度。如果值小于等于 0 则表示和实际控制台宽度的差。
 * @return 返回包含每行内容的数组。
 */
function splitLog(value, width = 0) {
    // width <= 0 时表示和实际控制台宽度的差。
    if (width <= 0) {
        width += process.stdout.columns || 80;
    }
 ...
 * @return 返回源码内容。
 */
function formatSource(source, width = 0, height = 3, showLine = true, showColumn = true, line, column, endLine, endColumn) {
    // 计算要显示的开始行号。
