...
class SourceMapBuilder {
     ...
     */
    getSource(generatedLine, generatedColumn, alignColumn = false, alignLine = false) {
        // 搜索当前行指定列的映射。
     ...
     */
    getAllGenerated(sourcePath, sourceLine, sourceColumn) {
        const result = [];
        const sourceIndex = this.indexOfSource(sourcePath);
        if (sourceIndex >= 0) {
     ...
     * @return 返回添加的映射点。
     */
    addMapping(generatedLine, generatedColumn, sourcePath, sourceLine, sourceColumn, name) {
        // 创建映射点。
        const mapping = {
     ...
     */
    applySourceMap(other, file = other.file) {
