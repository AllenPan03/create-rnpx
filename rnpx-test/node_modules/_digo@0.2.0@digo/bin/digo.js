...
function main() {
    ...
    const options = {
        ...
        "--filter": {
            ...
            execute(path) { configs.push(["filter", path]); }
        },
        "-i": "--ignore",
        "--ignore": {
            description: "Add an ignore pattern.",
            execute(path) { configs.push(["ignore", path]); }
        },
        "--ignore-file": {
            description: "Load specified ignore file. e.g. '--ignore-file .gitignore'.",
            execute(path) { configs.push(["ignoreFile", path]); }
        },
        "--encoding": {
            description: "Specify the default encoding. Defaults to 'utf-8'.",
            execute(encoding) { configs.push(["encoding", encoding]); }
        },
        "--overwrite": {
        ...
        "--source-map-root": {
            ...
            execute(prefix) { configs.push(["sourceMapRoot", prefix]); }
        },
    };
    // 解析命令行参数。
    let taskName;
    ...
    if (matchedTasks.length !== 1) {
        ...
        if (matchedTasks.length) {
            const tasksList = { __proto__: null };
    ...
    function startDebugger(breakOnEntry, debugPort) {
        ...
        if (process.execArgv.indexOf("--harmony") >= 0)
            argv.unshift("--harmony");
        if (process.execArgv.indexOf("--experimental-modules") >= 0)
            argv.unshift("--experimental-modules");
        if (process.execArgv.indexOf("--no-warnings") >= 0)
            argv.unshift("--no-warnings");
